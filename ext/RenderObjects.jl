...
- `edges = nothing`: draws the feature edges of the object, i.e. the sharp edges and the boundary of
  open surfaces, as thin dark lines. By default only in the `:cad` look, see [`set_render_look`](@ref)
...
function render!(ax::_RenderEnv, ::BMO.SingleShape, obj; material = nothing,
    ...
    edges = something(edges, _default_edges() && !mechanics)
...
"""
    render!(ax, ::MultiShape, obj; edges = _default_edges(), kwargs...)
...
"""
function render!(ax::_RenderEnv, ::BMO.MultiShape, obj; edges::Bool = _default_edges(), kwargs...)
