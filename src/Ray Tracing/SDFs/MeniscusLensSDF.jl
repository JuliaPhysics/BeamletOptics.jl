...
"""
mutable struct MeniscusLensSDF{T} <: AbstractRotationallySymmetricSDF{T}
    dir::SMatrix{3, 3, T, 9}
    transposed_dir::SMatrix{3, 3, T, 9}
    pos::Point3{T}
    convex::ConvexSphericalSurfaceSDF{T}
...
function MeniscusLensSDF(r1::R1, r2::R2, l::L, d::D = 1inch) where {R1, R2, L, D}
    ...
    concav_surface = SphereSDF(concav_radius)
    ...
    if orientation == :left_facing
        ...
        translate3d!(concav_surface, [0, concav_radius + l, 0])
    elseif orientation == :right_facing
        translate3d!(concav_surface, [0, -concav_radius, 0])
        translate3d!(cylinder, [0, -concav_sag, 0])
        zrotate3d!(convex_surface, π)
        translate3d!(convex_surface, [0, thickness(cylinder) - concav_sag + convex_sag, 0])
    else
        throw(ErrorException("Something went very wrong ... :)"))
    end
    # Return shape
    return MeniscusLensSDF{T}(
        Matrix{T}(I, 3, 3),
        Matrix{T}(I, 3, 3),
        Point3{T}(0),
        convex_surface, cylinder, concav_surface,
        l
    )
end
