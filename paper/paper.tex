...
architecture and the ray and beamlet models it implements, showcase a 
...
electromagnetic ($\mathrm{TEM}_{00}$) mode, utilization of the Gaussian
...
(colored) through the collection optics onto the sensor plane (blue).
...
tunneling through geometry. Steps are large in open space and shrink as
...
the element count, i.e. $\mathcal{O}(n)$.
...
Geometrical rays cannot by themselves describe the Gaussian mode of a laser, 
...
beam cannot be clipped by hard apertures. Within
...
Each beamlet consists of a polarized chief ray and eight auxiliary
...
The following mathematical description of the field reconstruction from a traced set of rays 
...
Eq.~\ref{eq:astigmatic-beamlet-field} omits the longitudinal
...
differences in capability between the two Gaussian beamlet models.
...
astigmatism that the three-ray model of Fig.~\ref{fig:beamlet} cannot
represent.}
\label{fig:agbeamlet}
\end{figure}
...
tracing, decomposition of an arbitrary field into smaller beamlets is
...
diffraction from complex apertures \cite{Ashcraft:2024, Greynolds:2014}.
...
a HeNe laser into a 1-inch \texttt{CubeBeamsplitter}; two
...
intersection tests, which currently scale as $\mathcal{O}(n)$ with the number $n$ of
...
for proofreading the manuscript. BeamletOptics.jl builds on the Julia language
\cite{bezanson2017julia} and the Makie visualization ecosystem
\cite{Danisch2021}.

\section*{Disclosures}

During the preparation of this manuscript, the authors used \mbox{Claude Opus 5
