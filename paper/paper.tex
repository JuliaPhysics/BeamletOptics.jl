...
complex-ray (\emph{Gaussian beamlet}) formalism of Arnaud and Greynolds,
so that the propagation of e.g. a laser beam through an arbitrarily
...
architecture, the ray and beamlet models it implements, and validate the
implementation against the analytical Fraunhofer diffraction pattern of
a double slit.
\end{abstract}
...
offers a variety of in-built features, such as speed, automatic
differentiation (AD) and easy access to multi-threading, that allow for
elegant and rapid iteration of algorithms.

BMO is developed at the Institute of Technical Physics (DLR), where one
...
three-dimensional breadboard setups with e.g. laser sources: one that
does not assume a fixed optical axis or surface order, lets the user
move components interactively, and still returns the physical-optics
quantities (waist, curvature, phase, interference) that matter for a
coherent source.

In Fig.~\ref{fig:openraman} the kind of setup is displayed that BMO
targets. The OpenRAMAN spectrometer \cite{OpenRAMAN} is traced using a
...
\centerline{\includegraphics[trim=100 10 35 10, clip,
...
shows the three ingredients of every BMO simulation: constructing
...
\centerline{\includegraphics[trim=32 38 30 38, clip,
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/sphere_tracing.png}}
...
algorithm \cite{Moeller:2005}, but with a per-face constant normal it is
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figures/iir_loop.pdf}}
\caption{The Intersect--Interact--Repeat loop. The inner black loop is
the per-ray trace; the outer red loop is the time-domain solution, in
...
is a responsibility of whoever implements such a composite element.
Systems without mutable elements can be marked \texttt{StaticSystem} for
...
moves are made with \texttt{translate3d!}, \texttt{rotate3d!} and the
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/beam_showcase.png}}
...
Geometrical rays can not by themselves describe the $\mathrm{TEM}_{00}$
Gaussian mode of a laser, whose radius, wavefront curvature and Gouy
phase evolve according to wave optics \cite{Saleh2019}. The standard
method already implemented as a Julia package is the $ABCD$
$q$-parameter method \cite{ABCDMatrixOptics}, which requires a paraxial
system matrix. BMO instead uses the \emph{complex ray} (or
ray-equivalent) method introduced by Arnaud \cite{Arnaud1968}: a
fundamental Gaussian can be represented by a small number of ordinary
geometrical rays traced through the optical system.

The \texttt{GaussianBeamlet} represents the mode with three coplanar
...
\theta = \frac{M^2 \lambda}{\pi w_0},
  \label{eq:divergence}
\end{equation}
where $M^2 \geq 1$ is the beam quality factor. After tracing all three
rays through the system, the local beam radius $w(z)$, radius of
curvature $R(z)$ and Gouy phase $\psi(z)$ are reconstructed from the ray
heights and slopes using the relations of Herloski \cite{Herloski1983}
and DeJager \cite{DeJager1992}. The complex field of the mode is then
\begin{equation}
E(r,z) = E_0\,\frac{w_0}{w(z)}\, e^{-r^2/w(z)^2}\, e^{\,i\left[kz +
\psi(z) + kr^2/2R(z)\right]},
  \label{eq:gaussfield}
\end{equation}
with $k = 2\pi/\lambda$. In addition, the optical path length of the
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/gaussian_showcase.png}}
\caption{A \texttt{GaussianBeamlet} focused by a single lens. The mode
...
For off-axis, tilted or otherwise non-symmetric systems the stigmatic
model breaks down \cite{Kochkina:2013}. Here BMO offers the
\texttt{AstigmaticGaussianBeamlet}, an implementation of the
generally-astigmatic beamlet formalism of Greynolds
\cite{Greynolds:1986_1, Greynolds:1986_2} in the vectorial,
polarization-aware form of Worku and Gross \cite{Worku:2017}. The mode
is carried by nine rays, see Fig.~\ref{fig:agbeamlet}: a polarized chief
ray plus four ``waist'' and four ``divergence'' rays sampling two
orthogonal transverse axes. Together they track the complex $2\times2$
curvature matrix $\mathbf{Q}(z) = \mathbf{U}\mathbf{H}^{-1}$, formed
from the transverse components of the complex ray heights
$\mathbf{H} = [\mathbf{h}_1, \mathbf{h}_2]$ and angles
$\mathbf{U} = [\mathbf{u}_1, \mathbf{u}_2]$. The diagonal of
$\mathbf{Q}$ gives the width and phase curvature along the principal
axes; the off-diagonal terms encode general astigmatism (wavefront
twist). Physical validity requires the \emph{complex optical invariant}
to vanish,
\begin{equation}
\mathbf{h}_1 \times \mathbf{u}_2 - \mathbf{h}_2 \times \mathbf{u}_1 = 0,
  \label{eq:invariant}
\end{equation}
which BMO checks after every interaction and which guarantees that
$\mathbf{Q}$ stays symmetric \cite{KochkinaAO:2013}. The reduced complex
amplitude of a single beamlet is
\begin{equation}
\psi(\mathbf{r}) = \frac{E_0}{\sqrt{\mathbf{h}_1 \times \mathbf{h}_2}}\,
\exp\!\left(\! ik\,
\frac{(\mathbf{h}_1\!\times\!\mathbf{r})(\mathbf{u}_2\!\cdot\!\mathbf{r})
-(\mathbf{h}_2\!\times\!\mathbf{r})(\mathbf{u}_1\!\cdot\!\mathbf{r})}
{2\,\mathbf{h}_1 \times \mathbf{h}_2} \right)\!,
  \label{eq:agbfield}
\end{equation}
evaluated with the analytic (non-conjugated) bilinear form to preserve
continuity in the complex domain \cite{Greynolds:2014, Wilhelm:2001}.
The physical $1/e^2$ intensity ellipse is recovered from the
eigendecomposition of the spot matrix
$\mathbf{S} = \operatorname{Re}(\mathbf{h}_1\mathbf{h}_1^\dagger + \mathbf{h}_2\mathbf{h}_2^\dagger)$
\cite{KochkinaAO:2013}. Related methods underpin e.g. the commercial
codes FRED and CODE~V and the open-source \texttt{poke}
\cite{Ashcraft:2022, Ashcraft:2024} and IfoCAD \cite{Wanner:2017}.
Table~\ref{tab:models} summarizes the difference in capability between
both beamlet models.

Beyond single modes, the astigmatic beamlet is also the building block
that makes \emph{beamlet decomposition} possible. Each beamlet stays a
valid, narrow solution under arbitrary tilt, astigmatism and off-axis
propagation, so an arbitrary wavefront can be expanded into a set of
them and propagated by ordinary ray tracing. This the basis of the field
decompositions of Section~\ref{sec:beamgroups}.

\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/agaussian_showcase.png}}
...
apertures, plus concave spherical, right-angle-prism and retroreflector
variants.
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/mobile_lens.png}}
...
\centerline{\includegraphics[trim=18 10 6 10, clip,
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/michelson_powerplot.png}}
...
from the path difference changing by $2\,\Delta y$; the finite minimum
...
\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{figures/agb_doubleslit.png}}
...
intersection tests, which scale as $\mathcal{O}(n)$ in the number $n$ of
...
algorithms run on the CPU. Tracing a \texttt{System} is deliberately
...
Brian Guenter for valuable technical discussions.
BeamletOptics.jl builds on the Julia language
