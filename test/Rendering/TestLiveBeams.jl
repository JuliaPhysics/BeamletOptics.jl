...
@testset "Live Gaussian beamlet rendering" begin
    ...
    @testset "AstigmaticGaussianBeamlet: not yet supported" begin
        agb = AstigmaticGaussianBeamlet([0, 0, 0], [0, 1, 0], 1000e-9, 1e-3)
        fig = Figure()
        ax = LScene(fig[1, 1])
        @test_throws ArgumentError Ext.live_render!(ax, agb)
    end
end
