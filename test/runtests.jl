...
@testset "Mesh/geometry backend" begin
    ...
    @test all(plane.geometry.pos .== pos + [0, 0, 1])

    SCDI.xrotate3d!(plane, π / 2)
    @test all(plane.geometry.vertices[2:3, 3] .== 2)

    SCDI.yrotate3d!(plane, π / 4)
    @test isapprox(plane.geometry.vertices[2, 3], 1 + sqrt(2), atol=5e-6)

    SCDI.zrotate3d!(plane, π / 2)
    @test isapprox(plane.geometry.vertices[3, 2], sqrt(2), atol=5e-6)

    SCDI.zrotate3d!(plane, -π / 2)
    SCDI.yrotate3d!(plane, -π / 4)
    SCDI.xrotate3d!(plane, -π / 2)
    SCDI.translate3d!(plane, [0, 0, -1])
    @test isapprox(plane.geometry.vertices, vertices, atol=1e-8)

    SCDI.scale3d!(plane, 2)
    @test isapprox(plane.geometry.vertices, vertices .* 2, atol=1e-8)
    @test plane.geometry.scale == 2
end